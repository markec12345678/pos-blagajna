...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        toast({ title: 'Napaka pri prijavi', description: data.error, variant: 'destructive' })
        return
      }
      toast({ title: 'Dobrodošli', description: `Prijavljeni kot ${data.user.name}` })
      router.push('/')
      router.refresh()
    } catch (e: any) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-900 p-4">
      <Card className="w-full max-w-md shadow-2xl">
        <CardHeader className="text-center pb-2">
          ...
          <CardTitle className="text-2xl">POS Blagajna</CardTitle>
          <p className="text-sm text-slate-500 mt-1">Restavracija Primorska</p>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-6 p-3 bg-slate-50 rounded-lg text-xs space-y-1.5">
            <div className="font-semibold text-slate-700 mb-1">Demo računi:</div>
